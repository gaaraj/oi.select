      ...
      compile: function(element, attrs) {
        ...
        return function(scope, element, attrs, ctrl) {
          ...
          scope.$watch('query', function(inputValue, oldValue) {
            ...
            if (String(inputValue).length < options.minlength) {
              return;
            }
          ...
          scope.$watch('groups', function(groups) {
            if (oiUtils.groupsIsEmpty(groups)) {
              ...
              if (options.clickedPlaceholder) {
                modifyPlaceholder(options.clickedPlaceholder);
              }
            }
          });
          ...
          scope.$watch('isFocused', function(isFocused) {
            ...
            if (!isFocused) { modifyPlaceholder(''); }
          });
          ...
          scope.removeItem = function removeItem(position) {
            if (attrs.disabled || multiple && position < 0) {
              return;
            }
          ...
          function click(event) {
            ...
            if (scope.query.length < options.minlength) {
              return;
            }

            //option is disabled
            if (oiUtils.contains(element[0], event.target, 'disabled')) {
              return;
            }

            //limit is reached
            if (scope.output.length >= multipleLimit && oiUtils.contains(element[0], event.target, 'select-dropdown')) {
              return;
            }
          ...
          function modifyPlaceholder(customText) {
            var currentPlaceholder = multiple && exists(ctrl.$modelValue) ? multiplePlaceholder : placeholder;
            // Is filtered ?
            if (typeof currentPlaceholder == "string") {
              var matches = currentPlaceholder.match(/(.*)\s?\|\s?(.*)/);
              if (matches) {
                console.info(matches);
                var val = matches[1].replace(/'/g, '');
                var filter = matches[2];
                var filtered = $filter(filter)(val);
                inputElement.attr('placeholder', filtered);
              } else {
                if (customText) {
                  inputElement.attr('placeholder', customText);
                } else {
                  inputElement.attr('placeholder', currentPlaceholder);
                }
              }
            } else {
              console.error("Placeholder type must be a string!");
            }
          }
